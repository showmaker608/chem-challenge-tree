...
export function TreeNode({ topic, difficulty, status, onClick }: TreeNodeProps) {
  const baseClass = "flex items-center gap-3 px-4 py-2.5 rounded-lg transition-all duration-200 cursor-pointer border";

  if (status === 'locked') {
    return (
      <div className={`${baseClass} bg-slate-800/30 border-slate-700/30 cursor-not-allowed opacity-50`}>
