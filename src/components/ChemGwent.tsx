...
export function ChemGwent({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { const action = game.duel ? chooseAI(game, { level: difficulty, style: game.opponentId }) : easyAI(game, lesson); const next = act(game, action); setGame(next); if (intro && next.phase === 'over') rememberIntro(); setNotice(describe(game, next, action)); respond(game, next); }, 1500);
