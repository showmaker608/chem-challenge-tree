...
export function ChemGwent({ onBack }: { onBack: () => void }) {
  ...
  function tile(c: Card, side?: Side, decorative = false) {
    ...
    return <button key={c.id} tabIndex={decorative ? -1 : undefined} aria-hidden={decorative || undefined} className={`cg-card cg-kind-${c.ability} ${selected===c.id?'selected':''} ${swapOut===c.id?'cg-picked-out':''} ${swapIn===c.id?'cg-picked-in':''} ${dealtId===c.id?'cg-dealt':''} ${eligible?'cg-targetable':''} ${target===c.id||replacement===c.id?'cg-target-chosen':''} ${state.startsWith('已')?'cg-used':''} ${burst && burst.side===side&&burst.cards.includes(c.id)?'cg-reacting':''}`} onClick={()=>{if(decorative)return;if(side===undefined&&game.phase==='mulligan'){setInspect(c);audio.current?.effect('select');return;}if(side===undefined){if(card?.skill==='relay'&&target&&c.ability==='unit'){setReplacement(c.id);}else{setSelected(c.id);setTarget(undefined);setReplacement(undefined);}}else if(eligible){setTarget(c.id);setReplacement(undefined);}else setInspect(c);audio.current?.effect('select');}} aria-label={`${c.name} ${value}分 ${state}${eligible?' 可选目标':''}${game.phase==='mulligan'?' 点击查看说明':''}`} aria-pressed={side===undefined?selected===c.id:eligible?target===c.id:undefined}><GwentCardFace card={c} value={value} state={state} /></button>;
  }
  ...
  return <main className="cg cg-beginner"><div className="cg-shell"><header className="cg-header"><button onClick={() => building ? setBuilding(false) : started ? game.phase === 'over' ? home() : setConfirmExit(true) : onBack()}>← 返回</button><div><small>CHEMICAL COMPANIONS</small><h1>化学对决</h1></div><nav><button aria-label="帮助" title="帮助" aria-expanded={rules} onClick={() => setRules(!rules)}>?</button><button aria-label="声音设置" title="声音设置" aria-expanded={settings} onClick={()=>setSettings(!settings)}>♫</button></nav></header>
    ...
    {building ? <GwentDeckBuilder initial={deckKeys} onSave={saveDeck} onClose={()=>setBuilding(false)} /> : !started ? <section className="cg-welcome"><span className="cg-kicker">CHEMICAL COMPANIONS</span><h2>让化学伙伴，<br />产生一点反应。</h2><p>巧妙组合，赢下这一局。</p><div className="cg-showcase" aria-label="化学伙伴卡牌预览">{['acid','peroxide','limewater'].map(chemical=>tile(createLesson('bond').players[0].hand.find(c=>c.chemical===chemical)!,undefined,true))}</div><div className="cg-home-actions"><button className="primary cg-start" onClick={()=>startQuick()}>开始对决<span aria-hidden="true">→</span></button><button className="cg-deck-link" onClick={()=>{setBuilding(true);setHomeNotice('');setRules(false);}}>我的牌组</button></div><div className="cg-difficulty" role="group" aria-label="对手强度"><span>对手强度</span><button aria-pressed={difficulty==='easy'} onClick={()=>pickDifficulty('easy')}>轻松 · 它会犯错</button><button aria-pressed={difficulty==='normal'} onClick={()=>pickDifficulty('normal')}>认真 · 全力计算</button></div>{homeNotice&&<p role="status" className="cg-home-notice">{homeNotice}</p>}</section> : <>
      ...
      <section className="cg-hand"><div className="cg-hand-head"><strong>你的手牌 · {game.players[0].hand.length}</strong><small>{game.phase==='mulligan'?'点卡看说明':'左右滑动查看 · 点击选牌'}</small></div><div className="cg-hand-cards">{game.players[0].hand.map(c => tile(c))}{!game.players[0].hand.length && <p>没有手牌了，点“本局不再出牌”进入结算。</p>}</div></section>
      ...
      {game.phase === 'play' && <div className="cg-actions"><div className="cg-preview">{card ? <><strong>{card.name} <button className="cg-info" onClick={()=>setInspect(card)} aria-label="查看卡牌说明">ⓘ</button></strong>{comboInfo(card.chemical).map(line => <span key={line} className="cg-preview-combo">{line}</span>)}<span>{targetHint}</span>{preview&&<span>你 {score(game,0)} → {score(preview,0)}{game.duel?` · 对手 ${score(game,1)} → ${score(preview,1)}`:''}</span>}{needsTarget&&target&&<button onClick={()=>{setTarget(undefined);setReplacement(undefined);}}>重选目标</button>}</> : <span>{canPlay ? '先点击一张手牌' : game.players[0].passed ? '已停牌，剩余手牌保留' : '等待电脑行动'}</span>}</div><button className="primary" disabled={!canPlay || !preview} onClick={() => playAction && dispatch(playAction)}>{needsTarget?'执行战术':playLabel}{preview&&!game.duel ? ` +${score(preview, 0) - score(game, 0)}` : ''}</button><button disabled={!canPlay} onClick={() => setConfirmPass(true)}>收手</button></div>}
    </>}
